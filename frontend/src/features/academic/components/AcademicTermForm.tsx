...
const academicTermSchema = z.object({
  academic_year_id: z.string().min(1, { message: 'Academic Year is required' }),
...
export type AcademicTermFormValues = z.infer<typeof academicTermSchema>;

interface AcademicTermFormProps {
  years: { id: string; name: string }[];
  initialData?: Partial<AcademicTermFormValues>;
...
export const AcademicTermForm: React.FC<AcademicTermFormProps> = ({
  ...
  isPending = false,
  errorMsg = null,
  submitLabel = 'Save Academic Term',
}) => {
  const {
    register,
    handleSubmit,
        ...
        <div className="flex items-center gap-6 pt-2">
          ...
          >
            ...
            <div className="flex flex-col">
              <span className="text-sm font-semibold text-slate-700">Set Active</span>
              <span className="text-xs text-slate-400">Makes this term currently active.</span>
            </div>
          </div>
