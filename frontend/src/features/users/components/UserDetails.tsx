...
export const UserDetails: React.FC = () => {
  ...
  const statusMutation = useMutation({
    ...
    onError: (err: any) => {
      setStatusError(err.response?.data?.detail || 'Failed to update user status.');
    },
  });
  ...
  if (error || !userDetail) {
    return (
      <div className="text-center py-20">
        ...
        <Link to="/administration/users" className="mt-6 inline-flex items-center gap-2 text-sm text-brand-600 font-semibold hover:underline">
  ...
  const handleStatusToggle = () => {
    statusMutation.mutate({ status: userDetail.status === 'active' ? 'inactive' : 'active' });
  };

  const handleSuspendToggle = () => {
    statusMutation.mutate({ status: userDetail.status === 'suspended' ? 'active' : 'suspended' });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {canUpdateUsers && (
          <button
            onClick={() => navigate({ to: '/administration/users/$userId/edit', params: { userId } as any })}
      ...
      {statusError && (
        <div className="p-4 bg-red-50 border border-red-100 rounded-xl text-red-700 text-sm">{statusError}</div>
      )}
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
          ...
          <div className="space-y-4 text-sm">
            <div className="flex justify-between items-center">
              ...
              <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold border ${
                userDetail.status === 'active' ? 'bg-emerald-50 text-emerald-700 border-emerald-100'
                : userDetail.status === 'inactive' ? 'bg-slate-100 text-slate-600 border-slate-200'
                : 'bg-red-50 text-red-700 border-red-100'
              }`}>
                <span className={`w-1.5 h-1.5 rounded-full ${
                  userDetail.status === 'active' ? 'bg-emerald-500'
                  : userDetail.status === 'inactive' ? 'bg-slate-400' : 'bg-red-500'
                }`} />
                {userDetail.status}
              </span>
            </div>
          ...
          {canUpdateUsers && (
            <div className="pt-6 border-t border-slate-100 space-y-2.5">
              ...
              >
                {userDetail.status === 'active' ? <><XCircle className="h-4 w-4" /> Deactivate Account</> : <><CheckCircle2 className="h-4 w-4" /> Activate Account</>}
              </button>
              <button
              ...
              >
                {userDetail.status === 'suspended' ? <><CheckCircle2 className="h-4 w-4" /> Lift Suspension</> : <><AlertCircle className="h-4 w-4" /> Suspend Account</>}
              </button>
            </div>
          )}
        </div>
          ...
          <div className="flex border-b border-slate-200 bg-slate-50/50">
            <button
              ...
              className={`px-6 py-4 font-bold text-sm border-b-2 transition-all flex items-center gap-2 cursor-pointer ${
                activeTab === 'roles' ? 'border-brand-500 text-brand-600 bg-white' : 'border-transparent text-slate-500 hover:text-slate-800'
              }`}
            >
                    ...
                    <div key={role.id} className="p-4 rounded-xl border border-slate-100 bg-slate-50/50 flex items-start justify-between gap-4">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {role.is_system_role && (
                            <span className="text-[9px] uppercase font-bold text-brand-600 tracking-wider bg-brand-50 px-1.5 py-0.5 rounded border border-brand-100">System</span>
                          )}
                        </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {userDetail.permissions.map((perm: any) => (
                      <div key={perm.id} className="p-3 rounded-lg border border-slate-100 bg-slate-50/30 flex flex-col">
                        ...
                        <span className="text-[9px] font-bold text-indigo-500 uppercase mt-1.5 tracking-wider">Category: {perm.category}</span>
                      </div>
                    ))}
                  </div>
                )}
