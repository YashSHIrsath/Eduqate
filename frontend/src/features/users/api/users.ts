...
export const createUser = async (payload: { email: string; role_ids: string[] }) => {
...
export const updateUserRoles = async (id: string, roleIds: string[]) => {
  const response = await apiClient.post(`/api/v1/users/${id}/roles`, { role_ids: roleIds });
  return response.data;
};

export const updateUserPermissions = async (id: string, permissionIds: string[]) => {
  const response = await apiClient.post(`/api/v1/users/${id}/permissions`, { permission_ids: permissionIds });
  return response.data;
};
